...
// 代表一个指标的labels
type Label struct {
	queryLabelNames    []string // 用于查询数据的条件标签
...
// 根据标签名, 获取所有标签的值
func (l *Label) GetValues(filters map[string]string, ins instance.KscInstance) map[string]string {
	...
	for _, name := range l.queryLabelNames {
...
func NewLabels(qln []string, iln []string, cl Labels) (*Label, error) {
