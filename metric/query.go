...
func GetPromMetricsByQueriesV2(queries []*Query, logger log.Logger) (pms map[string][]prometheus.Metric, err error) {
	if len(queries) <= 0 {
	...
	pms, err = GetLatestPromMetricsV2(queries[0].repo, queryMetrics, logger)
	if err != nil {
		return
	}
	return
